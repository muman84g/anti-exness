    ...
    def _record_command_health(
        ...
        if error is None:
            if active is not None:
                logging.info(
                    "S24 bridge recovered: command=%s outage_seconds=%.3f attempts=%d last_error=%s recovery_elapsed_seconds=%.3f",
                    command,
        ...
        if active is None:
            ...
            logging.error(
                "S24 bridge command failed: command=%s error=%s elapsed_seconds=%.3f consecutive_failures=1",
                command,
                error,
                elapsed,
            )
        ...
        if should_log:
            ...
            logging.warning(
                "S24 bridge command still failing: command=%s outage_seconds=%.3f attempts=%d last_error=%s elapsed_seconds=%.3f",
                command,
                max(0.0, finished_at - float(active["started_at"])),
                int(active["attempts"]),
                error,
        ...
        with self._command_lock:
            ...
            try:
                ...
                try:
                    ...
                    expires_epoch = int(time.time() + timeout_value + 1.0)
                    framed_command = f"REQ|{request_id}|{expires_epoch}|{cmd_str}"
