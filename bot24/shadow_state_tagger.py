# -*- coding: utf-8 -*-
"""Passive causal state tags for bot24 forward opportunities.
...
UTC = timezone.utc
TAGGER_VERSION = "s24_shadow_state_tagger_v1"


def _local_artifact_name(value: Any) -> str:
    ...
    if (
        ...
        raise ValueError("invalid shadow state tag CSV name")
    return name
...
def _validate_header(path: Path) -> None:
    ...
    with path.open("r", newline="", encoding="utf-8") as handle:
        ...
        for row in reader:
            if len(row) != len(TAG_FIELDS):
                raise RuntimeError(f"shadow state tag CSV row width mismatch: {path}")
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(cfg.get("enabled", False), bool):
            raise ValueError("shadow state tagger enabled must be boolean")
        self.enabled = bool(cfg.get("enabled", False))
        self.symbol = str(symbol)
        self.path = Path(log_dir) / _local_artifact_name(cfg.get("csv", "s24_shadow_state_tags.csv"))
        ...
        with self.path.open("r", newline="", encoding="utf-8") as handle:
            ...
            for row in csv.DictReader(handle):
                if None in row or any(value is None for value in row.values()):
                    raise RuntimeError(f"shadow state tag CSV row width mismatch: {self.path}")
                opportunity_id = str(row.get("opportunity_id") or "")
                if (
                    not opportunity_id
                    or row.get("tagger_version") != TAGGER_VERSION
                    or row.get("symbol") != self.symbol
                ):
                    raise RuntimeError(f"shadow state tag CSV row invalid: {self.path}")
        ...
        *,
        ...
        for column in ("Open", "High", "Low", "Close"):
            if column not in frame:
                raise ValueError("shadow state tagger requires OHLC columns")
        for _, row in frame.iterrows():
            row_open = _finite(row.get("Open"))
            row_high = _finite(row.get("High"))
            row_low = _finite(row.get("Low"))
            row_close = _finite(row.get("Close"))
            if None in {row_open, row_high, row_low, row_close} or float(row_low) > float(row_high):
                raise ValueError("shadow state tagger requires finite, ordered OHLC history")
