...
def clean_sync_block_if_flat(
    *,
    ...
    if auto_clear:
        ...
        related_ticket_raw = details.get("ticket") or details.get("order_ticket") or 0
        try:
            if isinstance(related_ticket_raw, bool):
                raise ValueError("boolean ticket identity")
            related_ticket = int(related_ticket_raw)
