...
def _windows_terminal_files_dirs():
    ...
    def score(path):
        ...
        has_s20_bridge = int(
            os.path.isfile(os.path.join(experts_dir, "BotBridge_s20.ex5"))
            or os.path.isfile(os.path.join(experts_dir, "BotBridge_s20.mq5"))
        )
        try:
            mtime = os.path.getmtime(path)
        except OSError:
            mtime = 0.0
        return (has_s20_bridge, mtime)
...
class EABridgeServer:
    def __init__(self, files_dir=None):
        ...
        self.cmd_file = os.path.join(
            self.bridge_dir, os.environ.get("EA_BRIDGE_COMMAND_FILE", "cmd_s20.txt")
        )
        self.res_file = os.path.join(
            self.bridge_dir, os.environ.get("EA_BRIDGE_RESPONSE_FILE", "res_s20.txt")
        )
        self.heartbeat_file = os.path.join(
            self.bridge_dir, os.environ.get("EA_BRIDGE_HEARTBEAT_FILE", "heartbeat_s20.txt")
        )
        self.lock_file = os.path.join(
            self.bridge_dir, os.environ.get("EA_BRIDGE_LOCK_FILE", "ea_bridge_s20.lock")
        )
