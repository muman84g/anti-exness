...
| bot21 / S21 | `exness-bot-21` | `bot21` | `bot関連backtest/backtest67_1_bot21` | `SOURCE_BACKTEST.md`: Ehlers top3 current implementation for `US500_137_1h`, `AUDUSD_021_1h`, `USDJPY_035_1h`. | Live-order params enabled. Current runner requires the updated `CLOSEDEAL` bridge; deployment, bridge compilation/attachment, service recreation, and noVNC account state remain separate runtime actions. |
...
| bot26 / S26 | `exness-bot-26` | `bot26` | `bot関連backtest/0_bot26実装PV2C520_OOS_001` | `SOURCE_BACKTEST.md`: USTEC/USOIL `PV2C520_DEVQ80_H75_FORWARD_R1`。signal decision後の即時Ask entry、actual fill基準75分hold。C4535 pendingは非稼働。 | User-authorized live (`live=true`, `shadow=false`)。Observed leakcheck反転悪化により2026-08-24に改善前へ復元。Deploy/restart/real order未実施。 |
| bot27 / S27 | `exness-bot-27` | `bot27` | `backtest/bot関連backtest/0_bot27実装PV2C859_OOS_001` | `SOURCE_BACKTEST.md`: USTEC `PV2C859_DEVQ825_H90_FORWARD_R1`。actual fill基準90分hold。C4566 volatility dwell exitは非稼働。 | User-authorized live (`live=true`, `shadow=false`)。Observed leakcheck反転悪化により2026-08-24に改善前へ復元。Deploy/restart/real order未実施。 |
| bot28 / S28 | `exness-bot-28` | `bot28` | `検討中/chatgpt案/0_bot28実装PV2C560_OOS_001` | `SOURCE_BACKTEST.md`: USTEC `PV2C560_DEVQ80_H75_FORWARD_R1`、threshold `0.1507958826882894`、decision基準75分hold。 | User-authorized live (`live=true`, `shadow=false`)。DEV固定選択、既観測診断は昇格根拠外。Deploy/restart/real order未検証。 |
| bot29 / S29 | `exness-bot-29` | `bot29` | `検討中/chatgpt案/0_bot29実装PV2C531_OOS_001` | `SOURCE_BACKTEST.md`: USTEC `PV2C531_CORRECTED_DIAGNOSTIC_R1`、ret25 positive + abs-return/TickVolume correlation high、decision基準60分hold。 | User-authorized live (`live=true`, `shadow=false`)。Deploy/restart/real order未検証。 |
