...
| bot23 / S23 | `exness-bot-23` | `bot23` | `bot23_loss_abort_structural_20260814` | `SOURCE_BACKTEST.md`: frozen XAUUSD M1 candidate `visual_loss_abort_g_failure_to_progress` (`man_028_v003`, hash `e1b7ac...`). | Live-enabled; per-bot host-only `startup.ini` is mounted to `/app/startup.ini`. Deploy/restart/EA attachment/runner start not performed. |
