...
| bot26 / S26 | `exness-bot-26` | `bot26` | `bot関連backtest/0_bot26実装PV2C520_OOS_001/best案` | `SOURCE_BACKTEST.md`: USTEC/USOIL C4535 v02。signal decision Askから60秒以内の+1 bps継続確認、single pending lane、actual fill基準75分hold。 | User-authorized live (`live=true`, `shadow=false`)。forward-only、leakcheck/holdout未使用。Deploy/restart/real order未検証。 |
| bot27 / S27 | `exness-bot-27` | `bot27` | `backtest/bot関連backtest/0_bot27実装PV2C859_OOS_001/良案_C4566_v01_gap補正` | `SOURCE_BACKTEST.md`: USTEC entry `PV2C859_DEVQ825_H90_FORWARD_R1`、exit `C4566_v01`。vol30 q2は連続20分、外側は累積10分、gap>15秒除外、hard hold 90分。 | User-authorized live (`live=true`, `shadow=false`)。forward-only。今回のdeploy/restart/real order未実施。 |
