...
| bot23 / S23 | `exness-bot-23` | `bot23` | `output/backtest213/candidates/bot23-x-archive-inventory-range-fade-opt-v001` + JST09-11 `stable_001-param-15-55-45` + JST11-13 `round_s2p5_d0p05_r0p03` | `SOURCE_BACKTEST.md`: forward-only ZA parentへ、JST09-11独立3-laneとJST11-13 round-level sweep独立1-laneを追加。11-13時案の固定runは`xau-jst1113-round5-parameter-shortlist-v001`、合算リスク監査は`bot23/evidence/jst0913_combined_v004`。 | ZA 4-lane（magics 230023-230026、各max2）とJST09-11（230027-230029）は不変。UTC02-04終端除外で2.5 USD節目、depth 0.05 ATR60、reclaim 0.03 ATR60、60分保有、0.01 lot、capacity 1をmagic 230030で運用。固定holdはbroker約定・quote時刻、spread defer、3 fresh poll、30分上限、10018再試行を永続化。将来用entry時計はLondon/New York DSTを別々に判定し、routingは無効。JST11-13 passive observerとticket/deal/exit-price監査を含め112 no-order回帰検証済み。新bridgeの再compile/attach、旧trades.csvのold移動、deploy/restartは未実施。 |
...
| bot25 / S25 | `exness-bot-25` | `bot25` | `検討中/chatgpt案/多重ポジ/20260827_XAUUSD_break比率wavefront` | `SOURCE_BACKTEST.md`: XAUUSD completed-M5 `V23 / combo_014_v001_v001`。man_231の両建てcore-satelliteを維持し、productive close後120分超のdrought中だけ少数側frontier addを抑制（仕様hash `12dc94...`）。 | 2026-08-28にローカルbot25をV23へ更新。`live=true` / `shadow=false`と既存の独立環境gateは維持。旧man_231 state-v5は厳密なIDとowned inventory照合後のみstate-v6へ互換移行する。同日にlot照合・重大block保持・durable CSVと、売買非介入のfrontier observer/taggerを追加。deploy/restart/runtime反映は別途確認する。 |
