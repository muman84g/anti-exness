...
| --- | --- | --- | --- | --- | --- |
| bot0 / dashboard | `bot0-dashboard` | `bot0` | not applicable | Read-only dashboard service. v3 has a bot23 adapter for `s23_params.json` and `s23_trades.csv`; realized PnL is deduplicated by confirmed close `deal_id`, split by live/shadow and strategy/signal, and MTM remains null without a read-only quote snapshot. Currency is required for PnL/PF aggregation. | Independent service; no broker bridge, command IPC, order permission, or live-bot state mutation. Default Compose bind is localhost `127.0.0.1:8230`; remote access requires an explicitly managed private boundary. CentOS pull/recreate is not performed by this change. |
