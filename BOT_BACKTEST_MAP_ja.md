...
| --- | --- | --- | --- | --- | --- |
| bot0 / dashboard | `bot0-dashboard` | `bot0` | not applicable | Read-only dashboard service. v1 has a bot23 adapter for `s23_params.json` and `s23_trades.csv`; realized PnL is deduplicated by confirmed close `deal_id`, and MTM remains null without a read-only quote snapshot. | Independent service; no broker bridge, command IPC, order permission, or live-bot state mutation. Default Compose bind is localhost `127.0.0.1:8230`; remote access requires an explicitly managed private boundary. CentOS pull/recreate is not performed by this change. |
