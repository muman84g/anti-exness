...
| bot18 / S18 | `exness-bot-18` | verify | `backtest33` / legacy `backtest24`, `backtest34` | `bot18_v2` README describes a 3-symbol ML event filter basket. `backtest33_lightGBM_template_audit_bot18v2` is the main known source. Legacy related folders include `backtest24_bot18` and `backtest34_bot18_small_range_filter`. | Running per user note. Local `docker-compose.yml` currently mounts `./bot18`, while this workspace has `bot18_v2` and no plain `bot18`; verify real deployment path before deploy/restart. |
