# -*- coding: utf-8 -*-
"""Bot25 V23 XAUUSD continuous bilateral core/satellite runner.
...
PREVIOUS_STRATEGY_ID = "bot25_man231_xauusd_bilateral_core_satellite_v001"
PREVIOUS_STRATEGY_KEY = "man231_bilateral_book"
...
class S25V23Runner:
    ...
    def _inventory_observer_context(
        ...
        core_positions = int(long_count > 0) + int(short_count > 0)
        ...
        return {
            ...
            "satellite_positions": max(0, len(positions) - core_positions),
        }
    ...
    def _load_state(self) -> dict[str, Any]:
        ...
        compatible_upgrade = (
            ...
            and state.get("strategy_id") == PREVIOUS_STRATEGY_ID
            and state_version == PREVIOUS_STATE_VERSION
            and isinstance(state.get("strategies"), dict)
            and isinstance(state["strategies"].get(PREVIOUS_STRATEGY_KEY), dict)
            and len(self.params.get("strategies", [])) == 1
        )
        if compatible_upgrade:
            target_key = self.params["strategies"][0]["id"]
            state["version"] = STATE_VERSION
            state["strategy_id"] = self.params["strategy_id"]
            state["strategies"] = {target_key: state["strategies"][PREVIOUS_STRATEGY_KEY]}
            target = state["strategies"][target_key]
            for key, value in self._default_strategy_state().items():
                target.setdefault(key, value)
            self._state_identity_status = "compatible_man231_to_v23"
            self._retired_state_sha256 = hashlib.sha256(raw or b"").hexdigest()
            logging.warning("S25 exact man_231 state staged for compatible V23 migration")
            return state
    ...
    def _retire_loaded_state_if_flat(self, strategy: dict[str, Any], quote_time: pd.Timestamp) -> bool:
        ...
        try:
            ...
            logging.exception("S25 new V23 state creation failed; restoring retired state")
    ...
    def _commit_compatible_state_upgrade(self, strategy: dict[str, Any], quote_time: pd.Timestamp) -> bool:
        """Persist the exact man_231 -> V23 state upgrade after owned sync, with CAS."""
        if self._state_identity_status != "compatible_man231_to_v23" or not self._retired_state_sha256:
            return False
        try:
            with open(STATE_FILE, "rb") as handle:
                current_raw = handle.read()
            ...
            return False
        if hashlib.sha256(current_raw).hexdigest() != self._retired_state_sha256:
            logging.critical("S25 man_231 state changed during V23 preflight; upgrade refused")
            return False
        digest_prefix = self._retired_state_sha256[:12]
        try:
            self._save_state()
        except Exception:
            logging.exception("S25 compatible V23 state save failed; old state preserved")
            return False
        self._state_identity_status = "current"
        self._retired_state_sha256 = None
        self._trade_row(
            ...
            reason="exact_man231_state_upgraded_to_v23",
            note=json.dumps({"source_version": PREVIOUS_STATE_VERSION, "sha256_prefix": digest_prefix}, sort_keys=True),
        )
        return True

    def _compatible_state_live_identity_proven(self, strategy: dict[str, Any]) -> bool:
        """Read-only proof that an upgraded live book is exactly the old owned book."""
        state = self._st(strategy)
        if not self.live_enabled:
            return all(bool(position.get("shadow")) for position in state.get("positions", []))
        if state.get("pending_open"):
            logging.critical("S25 V23 migration refused while an old OPEN reservation is unresolved")
            return False
        symbol = str(self.params.get("mt5_symbol", self.params["symbol"]))
        positions = self.executor.get_positions(symbol, int(strategy["magic"]))
        orders = self.executor.get_orders(symbol, int(strategy["magic"]))
        if positions is None or orders is None or orders:
            logging.critical("S25 V23 migration requires available positions and an empty owned order set")
            return False
        if any(not self._owned_position(strategy, position) for position in positions):
            logging.critical("S25 V23 migration found an unexpected same-magic position")
            return False
        ...
        live_by_id = {
            int(getattr(position, "identifier", 0) or position.ticket): position
            for position in positions
        }
        if set(state_by_id) != set(live_by_id):
            logging.critical("S25 V23 migration state/live owned-position IDs differ")
            return False
        return all(
            self._state_matches_live(strategy, state_position, live_by_id[position_id])
            for position_id, state_position in state_by_id.items()
        )
    ...
    def connect_and_preflight(self) -> bool:
        ...
        if self._state_identity_status == "foreign_or_invalid":
            logging.critical("Archive retired bot25 state before starting V23")
            return False
        ...
        if self._state_identity_status == "retired_bot25":
            if not self._retire_loaded_state_if_flat(strategy, quote_time):
                return False
        if self._state_identity_status == "compatible_man231_to_v23":
            if not self._compatible_state_live_identity_proven(strategy):
                return False
    ...
    def run_once(self) -> None:
        ...
        if now - self._last_status_log >= float(self.params.get("status_log_interval_seconds", 300)):
            long_count, short_count = self._position_counts(strategy)
            logging.info("S25 V23 status live=%s shadow=%s long=%d short=%d wave=%s drought_blocks=%d sync_block=%s", self.live_enabled, self.shadow_enabled, long_count, short_count, self._st(strategy).get("active_wave"), int(self._st(strategy).get("drought_blocked_adds", 0)), self._st(strategy).get("sync_block_reason"))
            self._last_status_log = now


# Compatibility alias for older test/evidence imports; runtime identity is V23.
S25Man231Runner = S25V23Runner
...
def self_test() -> None:
    ...
    assert compatible_runner._state_identity_status == "compatible_man231_to_v23"
    assert compatible_runner.connect_and_preflight(), "exact flat man_231 state must upgrade in place"
    ...
    try:
        ...
        assert live_upgrade.connect_and_preflight(), "exact owned man_231 inventory must migrate without flattening"
        assert live_upgrade._state_identity_status == "current"
        assert live_upgrade._position_counts(strategy) == (1, 0)
        assert live_upgrade._st(strategy)["positions"][0]["owner_comment"] == "s25_m231_L0490"
...
def main() -> int:
    ...
    if args.self_test:
        ...
        print("s25 V23 self-test ok")
        return 0
    runner = S25V23Runner(params)
