...
def cycle531_latest(target_bars: pd.DataFrame, strategy: dict[str, Any]) -> dict[str, Any]:
    bars = _utc_bars(target_bars)
    close = bars["Close"].astype(float)
    activity_column = "TickVolume" if "TickVolume" in bars.columns else "Volume"
    tick_volume = bars[activity_column].astype(float)
    log_return = np.log(close).diff()
    feature = log_return.abs().rolling(60, min_periods=30).corr(tick_volume)
    ...
    return {
        ...
        "ret25": ret_value,
        "absret_activity_corr60": feature_value,
        "threshold": threshold,
...
def latest_signal(
    ...
    if strategy_type == "PV2C531":
        return cycle531_latest(target_bars, strategy)
