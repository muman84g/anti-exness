...
from protocol_v2_strategy import cycle531_latest, latest_signal


def resolve_source(root: Path) -> Path:
    matches = [path for path in (root / "backtest").glob("*/*/*PV2C531_OOS_001*") if path.is_dir()]
    if len(matches) != 1:
        raise RuntimeError(f"PV2C531 source resolution failed: {matches}")
    return matches[0]
...
def main() -> int:
    ...
    params = json.loads((Path(__file__).parent / "s29_params.json").read_text(encoding="utf-8"))
    strategy = params["strategy"]
    assert params["require_midpoint_close"] is True
    assert params["max_signal_delay_seconds"] == 45
    assert strategy["strategy_type"] == "PV2C531"
    assert strategy["candidate"] == "PV2C531_CORRECTED_DIAGNOSTIC_R1"
    ...
    for row in samples.itertuples(index=False):
        ...
        result = cycle531_latest(bars.loc[:signal_time], strategy)
        assert abs(result["ret25"] - float(row.feature_ret25)) < 1e-12
        assert abs(result["absret_activity_corr60"] - float(row.feature_value)) < 1e-12
    ...
    print(f"bot29 PV2C531 porting parity ok: samples={len(samples)}")
    return 0
