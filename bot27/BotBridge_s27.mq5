...
#ifndef BRIDGE_NAME
#define BRIDGE_NAME "BotBridge_s27"
#endif
#ifndef BRIDGE_VERSION
#define BRIDGE_VERSION "2026-08-21-s27-midclose-v1"
#endif
#define BRIDGE_COMMANDS "ECHO,CAPS,ACCOUNT,INFO,HIST,OPEN,PENDING,POSITIONS,POSITION,ORDERS,CLOSEDEAL,MODIFY,CANCEL,CLOSE"

#ifndef BRIDGE_COMMAND_FILE
#define BRIDGE_COMMAND_FILE "cmd_s27.txt"
#endif
#ifndef BRIDGE_RESPONSE_FILE
#define BRIDGE_RESPONSE_FILE "res_s27.txt"
#endif
input string InpCommandFile = BRIDGE_COMMAND_FILE;
input string InpResponseFile = BRIDGE_RESPONSE_FILE;
...
string HandleCommand(const string command)
{
   ...
   if(op == "HIST" && n >= 4)
   {
      ...
      string response = "OK";
      double hist_point = SymbolInfoDouble(symbol, SYMBOL_POINT);
      for(int i = copied - 1; i >= 0; --i)
      {
         string bar_time = TimeToString(rates[i].time, TIME_DATE | TIME_MINUTES);
         double mid_close = rates[i].close + ((double)rates[i].spread * hist_point / 2.0);
         response += "|" + StringFormat("%s,%.10f,%.10f,%.10f,%.10f,%I64d,%.10f",
            bar_time,
            rates[i].open,
            rates[i].high,
            rates[i].low,
