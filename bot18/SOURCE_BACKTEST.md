...
While bot18 has bot-owned positions in one direction for a symbol, opposite-side virtual triggers do not become real market entries. The source exporter `export_bot18_cycle_events.py` mirrors this with `prevent_opposite_exposure=True`, while retaining `trigger_rearm_on_recross=True`. In the default `price_path_mode="close"` diagnostic, this means an order can fill only after the previous evaluated close has returned to the non-trigger side and the current evaluated close crosses the trigger again, and only if it would not create simultaneous LONG/SHORT exposure inside the same symbol cycle. Tick-exact live fills can still differ from M1-close diagnostics because live sends market orders from the current Bid/Ask when an eligible trigger is crossed.
