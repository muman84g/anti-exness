...
- While bot18 already has bot-owned positions in one direction for a symbol, only same-direction virtual triggers may become real market entries. Opposite-side virtual triggers remain local cycle state but are blocked from sending `OPEN` until the symbol is flat, so a single bot18 symbol does not intentionally hold LONG and SHORT at the same time.
