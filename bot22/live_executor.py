...
TRADE_PERMISSION_RETCODES = {"10026", "10027"}
...
class OrderInfo:
    ...
    def from_record(cls, record):
        ...
        if len(parts) < 9:
            raise ValueError(f"Invalid order record: {record}")
        return cls(*parts[:9])
...
class MT5Executor(BaseExecutor):
    ...
    def get_bridge_capabilities(self):
        ...
        if not res or not res.startswith("OK|CAPS|"):
            logging.critical(
                ...
                "BotBridge_s22.ex5 is likely stale, not attached, or not using the same Files directory.",
                res,
            )
            return None
        parts = res.split("|", 4)
    ...
    def get_account_info(self):
        """Return account execution metadata without exposing login or server IDs."""
        res = ea_bridge.send_command("ACCOUNT|")
        if not res or not res.startswith("OK|"):
            logging.critical("EA failed to get account execution info: %s", res)
            return None
        parts = res.split("|")
        if len(parts) < 7:
            logging.critical("EA returned malformed account info: %s", res)
            return None
        try:
            return {
                "margin_mode": int(parts[1]),
                "margin_mode_name": parts[2],
                "account_trade_allowed": bool(int(parts[3])),
                "account_trade_expert": bool(int(parts[4])),
                "terminal_trade_allowed": bool(int(parts[5])),
                "mql_trade_allowed": bool(int(parts[6])),
            }
        except (TypeError, ValueError) as exc:
            logging.critical("EA returned unparsable account info: %s (%s)", res, exc)
            return None
    ...
    def get_orders(self, symbol, magic=None):
        """Returns all live MT5 pending orders for symbol. None means bridge failure."""
        magic_filter = -1 if magic is None else int(magic)
        ...
        for record in parts[1:]:
            if not record:
                continue
            try:
                orders.append(OrderInfo.from_record(record))
            except Exception as e:
    ...
    def open_position(self, symbol, order_type, lot_size, sl=0.0, tp=0.0, deviation=20, magic=123456, comment="", digits=None):
        ...
        res = ea_bridge.send_command(
            f"OPEN|{symbol}|{order_type}|{lot_size}|{sl_text}|{tp_text}|{int(magic)}|{safe_comment}|{int(deviation)}"
        )
        ...
        try:
            parts = res.split("|")
            if len(parts) < 5:
                raise ValueError(f"malformed OPEN OK response: {res}")
            ticket_id = int(parts[1])
            deal_id = int(parts[2])
            exec_price = float(parts[3])
            retcode = parts[4]
            if ticket_id <= 0 or deal_id <= 0 or exec_price <= 0:
                raise ValueError(f"non-filled OPEN OK response: {res}")
        except (TypeError, ValueError) as exc:
            self.last_order_error = f"MALFORMED_OK:{res}"
            logging.error("EA returned unparsable OPEN success for %s: %s", symbol, exc)
            return None

        ticket = Ticket(ticket_id, exec_price)
        ticket.deal = deal_id
        ticket.retcode = retcode
        logging.info(f"Order filled via EA / Ticket: {ticket} (Price: {ticket.price})")
        return ticket

    def modify_position_sl_tp(self, ticket, sl=0.0, tp=0.0, digits=5):
        """Updates server-side SL/TP for an open position."""
        sl_text = f"{float(sl):.{int(digits)}f}" if sl else "0"
        tp_text = f"{float(tp):.{int(digits)}f}" if tp else "0"
    ...
    def close_position(self, ticket, deviation=20):
        ...
        res = ea_bridge.send_command(f"CLOSE|{ticket}|{int(deviation)}")
        ...
        if res and res.startswith("OK|"):
            ...
            try:
                parts = res.split("|")
                if len(parts) < 8:
                    raise ValueError(f"malformed CLOSE OK response: {res}")
                lot = float(parts[2])
                ...
                logging.error("EA returned unparsable CLOSE success for %s: %s", ticket, exc)
                return CloseResult(False, status="MALFORMED_OK")
