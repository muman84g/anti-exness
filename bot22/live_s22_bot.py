# -*- coding: utf-8 -*-
"""S22 EURUSD H1 Bollinger squeeze-breakout pullback live/shadow runner.

Source candidate: man_024_v002 / EURUSD_005_1h / f97149f97d028e98.
"""
...
LOG_FILE = os.path.join(LOG_DIR, "s22_bot.log")
TRADE_LOG_FILE = os.path.join(LOG_DIR, "s22_trades.csv")
TRADE_ERROR_FILE = os.path.join(LOG_DIR, "s22_trade_errors.csv")
STATE_FILE = os.path.join(STATE_DIR, "s22_bot_state.json")
PARAMS_FILE = os.path.join(SCRIPT_DIR, "s22_params.json")
...
def latest_squeeze_pullback_signal(bars: pd.DataFrame, spec: dict[str, Any]) -> dict[str, Any] | None:
    bb_period = int(spec["bb_period"])
    width_lookback = int(spec["width_lookback"])
    pullback_window = int(spec["pullback_window"])
    min_len = bb_period + width_lookback + pullback_window + 2
    if len(bars) < min_len:
        return None
    ...
    upper = ma + float(spec["std_mult"]) * std
    lower = ma - float(spec["std_mult"]) * std
    width = (upper - lower) / ma.replace(0.0, float("nan"))
    squeeze_level = width.rolling(width_lookback).quantile(float(spec["squeeze_quantile"])).shift(1)
    squeezed = width.shift(1) <= squeeze_level
    long_break = squeezed & (close > upper)
    short_break = squeezed & (close < lower)
    recent_long_break = long_break.shift(1).rolling(pullback_window).max().fillna(0).astype(bool)
    recent_short_break = short_break.shift(1).rolling(pullback_window).max().fillna(0).astype(bool)

    i = len(bars) - 1
    if any(pd.isna(series.iloc[i]) for series in (ma, upper, lower, width, squeeze_level)):
        return None
    long_sig = (
        bool(recent_long_break.iloc[i])
        and low.iloc[i] <= ma.iloc[i]
        and close.iloc[i] > ma.iloc[i]
        and close.iloc[i] > open_price.iloc[i]
    )
    short_sig = (
        bool(recent_short_break.iloc[i])
        and high.iloc[i] >= ma.iloc[i]
        and close.iloc[i] < ma.iloc[i]
        and close.iloc[i] < open_price.iloc[i]
    )
    ...
    return {
        ...
        "squeeze_level": float(squeeze_level.iloc[i]),
    }
...
def comment_prefix(spec: dict[str, Any]) -> str:
    return str(spec.get("comment_prefix") or f"s22_{symbol_key(spec).lower()}")[:20]


class S22SqueezePullbackRunner:
    ...
    def _notify_reconciliation_required(self, spec: dict[str, Any], reason: str, details: dict[str, Any]) -> None:
        ...
        self._notify_manual_action(
            ...
            action="Inspect MT5 positions/orders and bot22 state/logs before clearing this block or enabling entries.",
            key=f"bot22:reconciliation:{symbol_key(spec)}:{reason}",
        )
    ...
    def _set_sync_block(
        ...
        *,
        ...
        if reason:
            ...
            if previous != reason:
                logging.error("S22 new entries blocked for %s: %s", symbol_key(spec), reason)
            if not recoverable:
                self._notify_reconciliation_required(spec, reason, clean_details)
            return
        if st.get("sync_block_new_entries"):
            logging.warning("S22 new-entry block cleared for %s after clean sync: %s", symbol_key(spec), previous)
    ...
    def _record_trade_permission_reject(self, spec: dict[str, Any], error: str | None) -> None:
        ...
        if streak >= notify_after and not bool(st.get("autotrading_reject_notified")):
            self._notify_manual_action(
                ...
                action="Enable MT5 Algo Trading / EA trading permission for BotBridge_s22, then inspect state and live exposure before clearing any block.",
                key=f"bot22:autotrading-disabled:{symbol_key(spec)}",
            )
    ...
    def _validate_lot(self, spec: dict[str, Any], info: Any) -> bool:
        ...
        if lot <= 0 or volume_min <= 0 or volume_max < volume_min or volume_step <= 0:
            logging.critical("S22 invalid volume metadata for %s: lot=%s min=%s max=%s step=%s", mt5_symbol(spec), lot, volume_min, volume_max, volume_step)
            return False
        if lot + 1e-12 < volume_min or lot - 1e-12 > volume_max:
            logging.critical("S22 lot outside broker limits for %s: lot=%s min=%s max=%s", mt5_symbol(spec), lot, volume_min, volume_max)
            return False
        nearest = round(lot / volume_step) * volume_step
        if abs(nearest - lot) > 1e-9:
            logging.critical("S22 lot does not align with broker step for %s: lot=%s step=%s", mt5_symbol(spec), lot, volume_step)
            return False
        return True
    ...
    def connect_and_preflight(self) -> bool:
        ...
        if not caps or caps["name"] != S22_BRIDGE_NAME:
            logging.critical("Bridge name mismatch: expected=%s got=%s", S22_BRIDGE_NAME, caps["name"] if caps else None)
            return False
        missing = REQUIRED_S22_COMMANDS - caps["commands"]
        if missing:
            logging.critical("S22 bridge missing commands: %s", sorted(missing))
            return False
        ...
        if self.live_enabled:
            if bool(self.params.get("require_hedging_account", True)) and int(account["margin_mode"]) != HEDGING_MARGIN_MODE:
                logging.critical(
                    "S22 live trading requires a hedging account for shared-account ownership safety: mode=%s",
            ...
            if not all(
                ...
                logging.critical("S22 live trading permissions are not fully enabled in MT5/account metadata.")
                return False
        for spec in self.params.get("symbols", []):
            ...
            if info is None:
                logging.critical("S22 symbol INFO preflight failed for %s", sym)
                return False
            if not self._validate_lot(spec, info):
                return False
            positions = self._owned_positions(spec)
            if positions is None:
                logging.critical("S22 POSITIONS preflight failed for %s", sym)
                return False
            ...
            if bars is None or bars.empty:
                logging.critical("S22 H1 HIST preflight failed for %s", sym)
                return False
    ...
    def _open_position(self, spec: dict[str, Any], info: Any, signal: dict[str, Any]) -> None:
        ...
        self._trade_row(
            ...
            sl=sl,
            tp=tp,
            reason="bollinger_squeeze_breakout_pullback",
            signal_bar_time=signal["bar_time"],
    ...
    def run_symbol_once(self, spec: dict[str, Any]) -> None:
        ...
        if bars is None:
            ...
            return
        signal = latest_squeeze_pullback_signal(bars, spec)
        if signal is None:
            return
    ...
    def run_once(self) -> None:
        if not bool(self.params.get("enabled", True)):
            logging.info("S22 disabled by params.")
            return
        for spec in self.params.get("symbols", []):
            try:
                self.run_symbol_once(spec)
            except Exception as exc:
                logging.exception("S22 symbol cycle failed for %s: %s", symbol_key(spec), exc)
    ...
    def _log_status(self) -> None:
        ...
        logging.info("S22 status: live=%s shadow=%s symbols=%s", self.live_enabled, self.shadow_enabled, compact)
    ...
    params["symbols"] = [
        {
            ...
            "symbol": "EURUSD",
            "mt5_symbol": "EURUSD",
            "spec_id": "EURUSD_005_1h",
            "comment_prefix": "s22_eurusd",
            "lot": 0.01,
            "bb_period": 20,
            "std_mult": 2.0,
            "width_lookback": 80,
...
class FakeOrder:
    def __init__(self, ticket: int, *, symbol: str = "EURUSD", magic: int = 200022, comment: str = "s22_eurusd_pending"):
        self.ticket = int(ticket)
        self.symbol = symbol
        self.type = 4
        self.direction = "LONG"
...
class FakeExecutor:
    ...
    def get_bridge_capabilities(self) -> dict[str, Any]:
        return {"name": S22_BRIDGE_NAME, "version": "selftest", "commands": REQUIRED_S22_COMMANDS}
    ...
    def open_position(self, *args: Any, **kwargs: Any) -> Any | None:
        ...
        if self.open_adds_position:
            self.positions.append(FakePosition(ticket, comment=kwargs.get("comment", "s22_eurusd_eurusd"), sl=0.0, tp=0.0))
...
def make_self_test_bars() -> pd.DataFrame:
    idx = pd.date_range("2026-01-01 00:00:00", periods=121, freq="1h")
    close = [1.1000 + (0.0008 if i % 2 else -0.0008) for i in range(121)]
    for i in range(90, 114):
        close[i] = 1.1000 + (0.00001 if i % 2 else -0.00001)
    close[114] = 1.1050
    close[115] = 1.1030
    close[116] = 1.1020
    close[117] = 1.1010
    ...
    for i, c in enumerate(close):
        row = {"Open": c, "High": c + 0.0002, "Low": c - 0.0002, "Close": c, "Volume": 100}
        if i == 114:
            row.update({"Open": 1.1002, "High": 1.1052, "Low": 1.1000})
        elif i == 119:
            row.update({"Open": 1.1002, "High": 1.1017, "Low": 1.0998})
        rows.append(row)
...
def make_self_test_runner(params: dict[str, Any], bars: pd.DataFrame, executor: FakeExecutor) -> S22SqueezePullbackRunner:
    runner = S22SqueezePullbackRunner(params)
...
def run_self_test() -> int:
    try:
        ...
        runner.run_once()
        active = runner.state["symbols"]["EURUSD"]["active"]
        ...
        unexpected = FakePosition(9102, magic=200022, comment="manual_eurusd")
        runner = make_self_test_runner(params, bars, FakeExecutor(positions=[unexpected]))
        runner.run_once()
        st = runner.state["symbols"]["EURUSD"]
        ...
        unexpected_order = FakeOrder(9202, magic=200022, comment="manual_pending")
        ...
        drift_pos = FakePosition(2200, comment="s22_eurusd_eurusd")
        ...
        runner.state["symbols"]["EURUSD"]["active"] = {
            ...
            "comment": "s22_eurusd_eurusd",
        }
        runner.run_once()
        assert runner.state["symbols"]["EURUSD"]["active"]["ticket"] == 2200, "ticket drift should recover a unique owned position"
        assert not runner.state["symbols"]["EURUSD"]["sync_block_new_entries"], "ticket drift recovery should not leave a block"
